...
def data_processing(data_name="FD001", smooth_param=1, exclude=False):
    ...
    用于根据给定FDOO序号生成数据集
    '''
    dir_path = '/home/ps/code/zhouzhihao/DCRAKAN-TEST/CMAPSSData/'
...
class SequenceDataset(Dataset):
    def __init__(self, mode='pretrain', win = 10, group=None, y_label=None, sequence_train=50, sequence_test=20, patch_size=10,engine_num=None):
        self.mode = mode
        X_ = []
        y_ = []
        time_stamp = []
        ...
        if mode == 'train':
            ...
            while i < self.unit_nr_total:
                ...
                for j in range(patch_size, length_cur_unit_nr-win+1):
                    channels = []
                    # 循环创建每个通道
                    for ii in range(win):
                        # 选择从第i列到第i+10列的数据作为通道
                        channels.append(self.x[ii+j - patch_size:j+ii, 2:-1])
                    # 将这些通道堆叠成一个三维数组
        ...
        if mode == 'test_all':
            ...
            while i <= self.unit_nr_total:
                ...
                # split by sequences
                length_cur_unit_nr = len(self.x)
                while length_cur_unit_nr - win +1 < patch_size+1:
                    self.x = np.vstack((self.x[0,:],self.x))
                    length_cur_unit_nr = len(self.x)
                ...
                for j in range(patch_size, length_cur_unit_nr - win + 1):
                    ...
                    for ii in range(win):
                        # 选择从第i列到第i+10列的数据作为通道
                        channels.append(self.x[ii + j - patch_size+1:j + ii+1, 2:])
                    X = np.stack(channels, axis=0)
                    X = X.astype(float)
                    X_.append(X)

                    y = length_cur_unit_nr - j + y_label[i - 1] - 1 - win + 1
        ...
        if mode == 'test_all_specific':
            ...
            while i <= self.unit_nr_total:
                ...
                for ii in range(win):
                    iii = ii-win+1
                    if iii < 0:
                        channels.append(data[iii - patch_size:iii, 2:])
                    else:
                        channels.append(data[- patch_size:, 2:])
                # 将这些通道堆叠成一个三维数组
                X = np.stack(channels, axis=0)
